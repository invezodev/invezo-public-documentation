---
title: "Introduction"
description: "Invezo is the first comprehensive data library for crypto assets."
---

## Getting Started

To get started with the Invezo, you must first [sign up for an account](https://app.invezo.com/signup). After signing up and verifying your email
address, navigate to the
["API Key" section](https://app.invezo.com/settings/keys) of the settings page.

If you have problems with retrieving your API key, please email
[support@invezo.com](mailto:support@invezo.com) and we will promptly resolve
your issue.

<Card
  title="API Documentation"
  icon={solid("rectangle-terminal")}
  href="/asset-reference/asset-lists"
>
  Jump straight to the documentation.
</Card>

<Card
  title="Invezo Intelligence Studio"
  icon={solid("chart-simple")}
  href="https://app.invezo.com/"
>
  Our web-based visual dashboard for metric analysis.
</Card>

<Card
  title="Our Slack Community"
  icon={brands("slack")}
  href="https://invezo-community.slack.com/join/shared_invite/zt-1ban1w2s2-zTzVI~diBTui9PlT~o02vw#/shared-invite/email"
>
  Join our Slack to get updates on what's new and get immediate support.
</Card>

<Card title="Support" icon={solid("envelope")} href="support@invezo.com">
  For support, please send us an email so we can fix your issue quickly
</Card>

<Card
  title="Institutional Plans"
  icon={solid("pencil")}
  href="https://docs.google.com/forms/d/e/1FAIpQLSfP1qmjQQaaOUorPR0e22TfKuuW2GUcADKJdLfa1CbEFDPxXw/viewform"
>
  To request an institutional plan, please fill out this form.
</Card>

## Social Links

<Card
  title="Twitter"
  icon={brands("twitter")}
  color="#179cf0"
  href="https://twitter.com/invezo_com"
>
  Follow @invezo_com for updates
</Card>

<Card
  title="Slack"
  icon={
    <svg
      className="w-6 h-6"
      viewBox="0 0 123 123"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M25.8 77.6002C25.8 84.7002 20 90.5002 12.9 90.5002C5.8 90.5002 0 84.7002 0 77.6002C0 70.5002 5.8 64.7002 12.9 64.7002H25.8V77.6002Z"
        fill="#E01E5A"
      />
      <path
        d="M32.3 77.6002C32.3 70.5002 38.1 64.7002 45.2 64.7002C52.3 64.7002 58.1 70.5002 58.1 77.6002V109.9C58.1 117 52.3 122.8 45.2 122.8C38.1 122.8 32.3 117 32.3 109.9V77.6002Z"
        fill="#E01E5A"
      />
      <path
        d="M45.2 25.8C38.1 25.8 32.3 20 32.3 12.9C32.3 5.8 38.1 0 45.2 0C52.3 0 58.1 5.8 58.1 12.9V25.8H45.2Z"
        fill="#36C5F0"
      />
      <path
        d="M45.2 32.2998C52.3 32.2998 58.1 38.0998 58.1 45.1998C58.1 52.2998 52.3 58.0998 45.2 58.0998H12.9C5.8 58.0998 0 52.2998 0 45.1998C0 38.0998 5.8 32.2998 12.9 32.2998H45.2Z"
        fill="#36C5F0"
      />
      <path
        d="M97 45.1998C97 38.0998 102.8 32.2998 109.9 32.2998C117 32.2998 122.8 38.0998 122.8 45.1998C122.8 52.2998 117 58.0998 109.9 58.0998H97V45.1998Z"
        fill="#2EB67D"
      />
      <path
        d="M90.5 45.2C90.5 52.3 84.7 58.1 77.6 58.1C70.5 58.1 64.7 52.3 64.7 45.2V12.9C64.7 5.8 70.5 0 77.6 0C84.7 0 90.5 5.8 90.5 12.9V45.2Z"
        fill="#2EB67D"
      />
      <path
        d="M77.6 97C84.7 97 90.5 102.8 90.5 109.9C90.5 117 84.7 122.8 77.6 122.8C70.5 122.8 64.7 117 64.7 109.9V97H77.6Z"
        fill="#ECB22E"
      />
      <path
        d="M77.6 90.5002C70.5 90.5002 64.7 84.7002 64.7 77.6002C64.7 70.5002 70.5 64.7002 77.6 64.7002H109.9C117 64.7002 122.8 70.5002 122.8 77.6002C122.8 84.7002 117 90.5002 109.9 90.5002H77.6Z"
        fill="#ECB22E"
      />
    </svg>
  }
  href="https://invezo-community.slack.com/join/shared_invite/zt-1ban1w2s2-zTzVI~diBTui9PlT~o02vw#/shared-invite/email"
>
  See what the Invezo Community is up to
</Card>

<Card
  title="Telegram"
  icon={brands("telegram")}
  color="#2ca5e0"
  href="https://t.me/invezo_com"
>
  Chat with us on Telegram
</Card>
